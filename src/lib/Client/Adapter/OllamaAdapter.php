...
class OllamaAdapter implements ProviderAdapterInterface
{
    private const DEFAULT_BASE_URL = 'http://localhost:11434/v1';
    private const DEFAULT_TEST_MODEL = 'llama3.1';

    public function supports(string $providerIdentifier): bool
    {
        return $providerIdentifier === 'ollama';
    }
    ...
    public function parseResponse(array $data): string
    {
        ...
        if ($text === '') {
            throw new \RuntimeException(
                sprintf('Ollama API returned an empty response. Raw: %s', json_encode($data))
            );
        }
